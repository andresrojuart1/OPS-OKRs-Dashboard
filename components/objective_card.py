...
def _kr_compact_bar(pct: float) -> str:
    ...
    return f"""<div style="background:rgba(255,255,255,0.06); border-radius:999px; height:6px; overflow:hidden; margin: 6px 0 16px 0;">
...
def _render_kr_block(data, active_kr: str) -> None:
    ...
    with st.container():
        ...
        # --- BOTTOM ACTION ROW ---
        b_left, b_right = st.columns([0.88, 0.12])
        with b_left:
