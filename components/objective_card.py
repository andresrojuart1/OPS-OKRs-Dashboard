"""Objective card — Ontop Fidelity Edition (Fixed Layout)."""
...
<span style="font-size:2.4rem; font-weight:800; color:white; letter-spacing:-0.05em;">{pct:.0f}%</span>
<div style="font-size:0.65rem; font-weight:700; color:{color}; background:rgba(255,255,255,0.05); padding:2px 8px; border-radius:999px; border:1px solid rgba(255,255,255,0.1); text-transform:uppercase;">Trend</div>
</div>"""

def _progress_bar(pct: float) -> str:
    return f"""<div style="background:rgba(255,255,255,0.05); border-radius:999px; height:6px; margin-top:10px; overflow:hidden;">
...
<div style="font-size:1.4rem; font-weight:700; color:white; margin-top:1.25rem; line-height:1.4;">{obj_title}</div>
        ...
        # Unified Compact Management Row
        c_actions = st.container()
        with c_actions:
            c1, c2, c3, _ = st.columns([0.45, 0.45, 1.3, 8])
            with c1:
...
<div style="font-size:1.1rem; font-weight:700; color:{GREEN};">{pct:.0f}%</div>
...
{_progress_bar(pct)}
    ...
    # REFINED ACTION RATIOS TO PREVENT VERTICAL TEXT
    c_upd, c_hist, c_ed, c_del, _ = st.columns([1.8, 1.2, 0.5, 0.5, 6])
    with c_upd:
        ...
        if st.button("Close" if is_upd else "Update Status", key=f"uk_{kr_id}", type="primary" if is_upd else "secondary", use_container_width=True):
            st.session_state["updating_kr"] = None if is_upd else kr_id
            st.rerun()
    with c_hist:
        if st.button("Logs", key=f"sh_{kr_id}", use_container_width=True):
...
def _render_update_form(kr) -> None:
    kr_id = str(kr["id"])
    st.markdown(f'<div style="background:rgba(255,255,255,0.02); padding:1.25rem; border-radius:12px; border:1px solid {PURPLE}33; margin:1rem 0;">', unsafe_allow_html=True)
    with st.form(key=f"f_{kr_id}", clear_on_submit=True):
        ...
        with c2: st.slider("Confidence", 1, 5, 3, key=f"cv_{kr_id}")
...
def _render_history(kr_id: str) -> None:
    ...
    for _, row in upds.iterrows():
        st.markdown(f'<div style="font-size:11px; color:{MUTED}; padding:8px 0; border-bottom:1px solid rgba(255,255,255,0.05);"><b>{row["new_value"]}</b> · {row["week_notes"]}</div>', unsafe_allow_html=True)
