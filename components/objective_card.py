"""Objective card — Fintech Edition (Final Rendering Fix)."""
...
text-transform:uppercase; letter-spacing:0.05em;">Trend</div>
</div>"""

def _fintech_progress_bar(pct: float) -> str:
    return f"""<div style="background:rgba(255,255,255,0.03); border-radius:999px; height:6px; margin-top:12px; position:relative; overflow:hidden;">
...
def render_objective_card(obj_row, krs_df, active_kr: str) -> None:
    ...
    # MUST START AT COLUMN 0 TO AVOID MARKDOWN CODE BLOCK TRIGGER
    st.markdown(f"""<div class="objective-card">
<div style="display:flex; align-items:center; gap:12px; margin-bottom:10px;">
<span style="font-size:0.7rem; font-weight:700; color:#7c73f7; background:rgba(124,115,247,0.1); padding:3px 8px; border-radius:4px; text-transform:uppercase; letter-spacing:0.05em;">Performance</span>
<span style="font-size:0.75rem; color:{MUTED}; font-weight:600;">{sub_team}</span>
</div>
{_pct_indicator(avg_pct)}
<div style="font-size:1.15rem; font-weight:700; color:white; margin-top:12px; line-height:1.4;">{obj_title}</div>
...
{_fintech_progress_bar(pct)}
    ...
    with c_edit:
        if st.button("✏️", key=f"ek_{kr_id}", type="tertiary"):
            _edit_kr_dialog(kr_id, title, target, unit)
...
def _render_update_form(kr) -> None:
    kr_id = str(kr["id"])
    st.markdown('<div style="background:rgba(255,255,255,0.02); padding:1rem; border-radius:12px; border:1px solid rgba(255,255,255,0.05); margin:8px 0;">', unsafe_allow_html=True)
    with st.form(key=f"f_{kr_id}", clear_on_submit=True):
        ...
        if st.form_submit_button("Save Update", use_container_width=True):
            ...
            update_kr_value(kr_id=kr_id, new_value=float(new_val), week_notes=notes, updated_by=email)
