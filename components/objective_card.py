"""Objective card — Fintech Container Edition."""
...
<span style="font-size:2.4rem; font-weight:800; color:{color}; letter-spacing:-0.05em;">{pct:.0f}%</span>
...
def render_objective_card(obj_row, krs_df, active_kr: str) -> None:
    ...
    # THE MAGIC WRAPPER: Native container + CSS Trigger
    with st.container():
...
<span style="font-size:0.7rem; font-weight:700; color:#7c73f7; background:rgba(124,115,247,0.1); padding:4px 10px; border-radius:6px; text-transform:uppercase; letter-spacing:0.04em;">Performance Index</span>
...
<div style="font-size:1.4rem; font-weight:700; color:white; margin-top:1.25rem; line-height:1.4;">{obj_title}</div>
        ...
        with c3:
            if st.button("+ Key Result", key=f"akr_{obj_id}", type="tertiary"): add_kr_dialog(obj_id, obj_title)

        st.markdown('<div style="margin-top:2rem;"></div>', unsafe_allow_html=True)

        # KRs inside the same container
        if not obj_krs.empty:
            for _, kr in obj_krs.iterrows():
                _render_kr_row(kr, active_kr)
        else:
            st.markdown(f'<div style="color:{MUTED}; padding:1rem; font-size:0.9rem;">No data entries yet.</div>', unsafe_allow_html=True)
...
<div style="font-size:1.1rem; font-weight:700; color:#2DD4BF;">{pct:.0f}%</div>
