...
def _kr_progress_row(pct: float, current_formatted: str, target_formatted: str) -> str:
    color = _pct_color(pct)
    return f"""
    <div style="margin: 4px 0 12px 0;">
        <div style="display:flex; justify-content:space-between; align-items:baseline; margin-bottom:6px;">
            <div style="font-size:18px; font-weight:800; color:{color};">{current_formatted}</div>
            <div style="font-size:12px; font-weight:500; color:rgba(255,255,255,0.4);">Target: {target_formatted}</div>
        </div>
...
def _render_kr_block(data, active_kr: str) -> None:
    ...
    with st.container():
        ...
        # --- HEADER ROW: Title (Left) + Settings (Right) ---
        ...
        # --- PROGRESS ROW: Value, Target, and Bar ---
        cur_fmt = latest.get("value_format", "number") if latest is not None else "number"
        curr_str = _format_badge(val, unit, cur_fmt)
        tgt_str = _format_badge(target, unit, cur_fmt).replace("+", "")

        st.markdown(_kr_progress_row(pct, curr_str, tgt_str), unsafe_allow_html=True)
